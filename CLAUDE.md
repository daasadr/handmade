...
⚠️ **Na produkci běží `NODE_ENV=production`, takže `synchronize` je VYPNUTÉ.**
Schéma se mění výhradně **migracemi** v `backend/src/migrations/`, které se spouští
automaticky při startu backendu (`migrationsRun: true`).

**Přidáš-li sloupec do entity, MUSÍŠ k němu napsat migraci.** Bez ní se sloupec
na produkci nevytvoří, ale TypeORM ho stejně vloží do každého SELECTu — dotaz
spadne na `column X does not exist`. U entity `User` to shodí i přihlášení.
(Stalo se 2026-07-21 u VIP sloupců, viz CHANGELOG.)

Formát migrace: okopíruj `1752401000000-AddCzechTranslations.ts`. Timestamp
v názvu musí být vyšší než u poslední existující migrace.
...
2. ~~**TypeORM migrace** místo `synchronize: development`~~ — ✅ **HOTOVO.**
   Server běží na `NODE_ENV=production`, migrace v `backend/src/migrations/`
   se spouští automaticky při startu. Viz varování v sekci 4 — každá změna
   entity potřebuje migraci.
